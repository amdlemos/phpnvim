...
vim.api.nvim_create_autocmd("FileType", {
	...
	local opts = { buffer = true, silent = true }
		...
		vim.keymap.set("n", "l", "<Plug>(DBUI_GotoChildNode)", vim.tbl_extend("force", opts, { desc = "DBUI: goto child/enter" }))
		-- h: collapse node if open, otherwise go to parent
		vim.keymap.set("n", "h", "<Plug>(DBUI_GotoParentNode)", vim.tbl_extend("force", opts, { desc = "DBUI: collapse node" }))
	end,
})
